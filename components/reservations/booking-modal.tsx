...
export function BookingModal({ slot, onClose, onSuccess }: Props) {
  ...
  function addGuest() {
    if (guests.length < 3) setGuests((prev) => [...prev, { name: '', email: '' }])
  }
    ...
    <>
      ...
      >
        ...
        >
          ...
          <h2 className="font-serif text-heading font-light text-navy mb-6">Book a Bay</h2>

          {/* Slot summary */}
          <div className="bg-cream-mid px-5 py-4 mb-6 grid grid-cols-3 gap-2">
            <div>
              <p className="font-mono text-label uppercase tracking-[0.22em] text-sage mb-1">Bay</p>
              <p className="font-serif text-xl font-light text-navy leading-tight">{slot.bayName}</p>
            </div>
            <div>
              <p className="font-mono text-label uppercase tracking-[0.22em] text-sage mb-1">Start</p>
              <p className="font-serif text-xl font-light text-navy leading-tight">
                {format(slot.startTime, 'h:mm a')}
              </p>
            </div>
            <div>
              <p className="font-mono text-label uppercase tracking-[0.22em] text-sage mb-1">Date</p>
              <p className="font-serif text-xl font-light text-navy leading-tight">
          ...
          {/* Duration selector */}
          <div className="mb-6">
            <p className="font-mono text-label uppercase tracking-[0.28em] text-sage mb-3">Duration</p>
            ...
            <div className="mt-2">
              <p className={['font-mono text-label tracking-[0.1em]', exceedsClose ? 'text-red-500' : 'text-sand'].join(' ')}>
                {exceedsClose
          ...
          <div className="mb-6">
            ...
            <div className="space-y-5">
              {guests.map((guest, i) => (
                <div key={i} className="relative border border-cream-mid px-4 pt-4 pb-3">
          ...
          <div className="flex gap-3">
            <Button onClick={handleSubmit} loading={isSubmitting} disabled={exceedsClose} className="flex-1">
              Confirm Booking
            </Button>
