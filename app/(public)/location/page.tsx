...
function StaticMap({
  ...
  const staticUrl =
    ...
    `&size=1280x640` +
    `&scale=2` +
    (markerColor ? `&markers=color:${markerColor}|${encoded}` : '') +
    MAP_STYLES +
    `&key=${apiKey}`
...
export default function LocationPage() {
  ...
  const westLAAddress = process.env.NEXT_PUBLIC_CLUB_ADDRESS ?? ''
  const eastSideAddress = 'Atwater Village, Los Angeles, CA'
      ...
      <div className='space-y-20'>
        ...
        <div className='space-y-10'>
          ...
          {key ? (
            <StaticMap
              apiKey={key}
              address={westLAAddress}
              markerColor='0xb8963c'
              alt='Fescue Golf Club — West LA'
              mapsUrl={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(westLAAddress)}`}
            />
          ) : (
            <MapPlaceholder />
          )}
        ...
        {/* ── 02 East Side LA (Coming Soon) ── */}
        <div className='space-y-10'>
          <div className='flex items-baseline gap-6'>
            <span className='font-mono text-label uppercase tracking-[0.28em] text-gold'>02</span>
            <div>
              <div className='flex items-center gap-3'>
                <p className='font-serif text-2xl font-light text-navy'>East Side LA</p>
                <span className='font-mono text-[10px] uppercase tracking-[0.2em] text-gold border border-gold/40 px-2 py-0.5'>
                  Coming Soon
                </span>
              </div>
              <p className='font-mono text-label uppercase tracking-[0.18em] text-navy/40 mt-1'>
          ...
          {key ? (
            <StaticMap
              apiKey={key}
              address={eastSideAddress}
              alt='Fescue Golf Club — East Side LA (Coming Soon)'
              mapsUrl={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(eastSideAddress)}`}
            />
          ) : (
            <MapPlaceholder />
          )}
